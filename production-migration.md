...
- テストサイト検索避け: `noindex,nofollow` 済み
...
1. テストサイトで表示と文言を最終確認する。
2. 問い合わせフォームの本番送信先を決める。
3. 会員資料PDFを新しい保管先へ移すか決める。
4. `noindex,nofollow` を本番公開直前に外す。
5. DNS切替前のバックアップを取る。
6. 本番ドメイン切替を実施する。
7. スマホ、PC、フォーム、SNSカード、検索避け解除を確認する。
